...
router.post('/', async (req, res) => {
  ...
  await registrarHistorial('personas', id, 'creado', { id, nombre });
...
router.put('/:id', async (req, res) => {
  ...
  await registrarHistorial('personas', Number(req.params.id), 'editado', { id: Number(req.params.id), nombre });
...
router.delete('/:id', async (req, res) => {
  const row = await db('personas').where('id', req.params.id).first();
  if (!row) return res.status(404).json({ error: 'Persona no encontrada' });
  await db('personas').where('id', req.params.id).del();
  await registrarHistorial('personas', row.id, 'eliminado', row);
